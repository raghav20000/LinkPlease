...
async def receive_webhook(request: Request, response: Response):
    ...
    # NOTE: currently log-only (warns, does not reject) -- see
    # FAILURES.md #1 for a documented, evidenced discrepancy between the
    # issued api_key and the signature the simulator actually sends.
    if settings.pseudogram_api_key:
        header_value = request.headers.get("X-PseudoGram-Signature")
        if not is_valid_signature(raw_body, header_value, settings.pseudogram_api_key):
            pass  # intentionally not rejecting -- see FAILURES.md #1
